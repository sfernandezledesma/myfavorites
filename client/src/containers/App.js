...
class App extends Component {
  ...
  changeLanguage = (languageCode) => {
    const currentLanguage = this.state.context.language;
    let newLanguage = languageCode;
    this.setState({ context: Object.assign({}, this.state.context, { language: newLanguage }) }, () => {
      if (this.state.searchField) {
        this.onSearch();
      }
    });
    console.log("Language changed to", newLanguage, "from", currentLanguage);
  }

  onSearch = () => {
    fetch(`/api/search/${this.state.context.language}/${this.state.searchField}`)
      ...
      .catch(err => this.setState({ errorMessage: String(err) }));
  }
